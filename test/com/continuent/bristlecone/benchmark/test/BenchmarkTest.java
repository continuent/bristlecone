...
/**
 * Basic unit test to confirm that benchmarks work. Test methods document
 * specific cases.
 * <p>
 * The test by default runs against Hypersonic. However, you can override this
 * by supplying a properties file name in the system property
 * "connection.properties". This properties file must define url, user, and
 * password values as shown in the following example:
 * <p>
 * <code><pre>
 * url=jdbc:postgresql://ubuntu4/mydb
 * user=benchmark
 * password=secret
 * </pre></code> To include the file invoke the test with
 * -Dconnection.properties=name where 'name' is the name of the file.
 * 
 * @author rhodges
 */
public class BenchmarkTest extends TestCase
{
    ...
    /**
     * Write the test case header. If there are connection properties load them
     * now.
     */
    protected void setUp() throws Exception
    {
        ...
        String connectionPropsName = System
                .getProperty("connection.properties");
        if (connectionPropsName != null)
        {
            PropertyManager pm = new PropertyManager();
            Properties connectionProperties = pm.loadProperties(new File(
                    connectionPropsName));
    ...
    public void testPropertyReplication() throws Exception
    {
        ...
        // Generate properties instances using a cross product and print the
        // result.
        PropertyManager pm = new PropertyManager();
        Vector<String> splitPropertyNames = new Vector<String>();
        Vector<Properties> crossProductList = pm.propertiesCrossProduct(
                testProps, null, splitPropertyNames);
        ...
        assertEquals("Incorrect split property names size",
                splitPropertyNames.size(), 1);
    ...
    public void testReplicationAndIncludes() throws Exception
    {
        ...
        PropertyManager pm = new PropertyManager();
        Vector<String> splitPropertyNames = new Vector<String>();
        Vector<Properties> crossProductList = pm.propertiesCrossProduct(f,
                splitPropertyNames);
        writePropertiesVector(crossProductList);
        ...
        // Ensure the split properties are as expected.
        assertEquals("Incorrect split property names size", 3,
                splitPropertyNames.size());
        String[] expectedNames = new String[]{"a", "b", "c"};
        for (int i = 0; i < expectedNames.length; i++)
        {
            assertTrue("Could not find expected split property: "
                    + expectedNames[i],
    ...
    public void testConfigMetadata1() throws Exception
    {
        ...
        assertEquals("setSimple() property call", "simple value",
                simple.getSimple());
    }

    /**
     * Demonstrate that ConfigMetadata can handle all supported setter argument
     * types
     */
    public void testConfigMetadata2() throws Exception
    {
        ...
        assertEquals("propString() property call", "propString1",
                complex.getPropString());
        assertEquals("propLong() property call", 100, complex.getPropLong());
        assertEquals("propInt() property call", (int) 13, complex.getPropInt());
        assertEquals("propBoolean() property call", true,
                complex.isPropBoolean());
        assertEquals("propChar() property call", 'b', complex.getPropChar());
        assertEquals("propDouble() property call", (float) 2.0,
                complex.getPropFloat());
        assertEquals("propString() property call", 1000.0,
                complex.getPropDouble());
    }
    ...
    public void testBenchmarkExecution2() throws Exception
    {
        ...
        this.runScenario("Default", ComplexScenario.class, new Properties(),
                true, 2);

        // Check that all methods were called once.
        assertEquals("Global init called once", 1,
                ComplexScenario.calledPrepare);
        assertEquals("Global cleanup called once", 1,
                ComplexScenario.calledGlobalCleanup);
    ...
    public void testBenchmarkExecution3() throws Exception
    {
        ...
        // Check that all methods were called once.
        assertEquals("Global init called once", 1,
                ComplexScenario.calledGlobalPrepare);
        assertEquals("Global cleanup called once", 1,
                ComplexScenario.calledGlobalCleanup);
     ...
     * least that length of time. This test is single threaded and does not run
     * multiple scenarios.
     */
    ...
    public void testBenchmarkExecution5() throws Exception
    {
        ...
        // Check that global methods and start/stop are called once.
        assertEquals("Global init called twice", 2,
                ComplexScenario.calledGlobalPrepare);
        assertEquals("Global cleanup called twice", 2,
                ComplexScenario.calledGlobalCleanup);
        assertEquals("Init called three times", 3,
                ComplexScenario.calledInitialize);
        assertEquals("Cleanup called three times", 3,
                ComplexScenario.calledCleanup);
        assertEquals("Iterate called 30 times", 30,
                ComplexScenario.calledIterate);
     ...
     * numbers of calls: <li>globalInit, globalCleanup -- Called once for each
     * run.</li> <li>init, cleanup -- Called once for each thread</li> </ul>
     */
    public void testBenchmarkExecution6() throws Exception
    {
        ...
        // Ensure that we iterated at least 1000 times. This is a fair smaller
        // number
     ...
     * replicaUrl for looking up inserted values. This case replicates common
     * usage for testing master/slave replication.
     * <p>
     * WARNING: This case may not work if you don't run testWriteSimpleScenario
     * first, because it reuses data from the previous case.
     * <p>
     */
    ...
    /**
     * Tests off-the-shelf query scenario with parameters designed to generate 9
     * lines of csv output (1 header row + 8 individual scenario runs).
     */
    ...
    /**
     * Tests off-the-shelf query long scenario with parameters designed to
     * generate 13 lines of csv output (1 header row + 12 individual scenario
     * runs).
     */
    public void testReadSimpleLargeResultsScenario() throws Exception
    {
        ...
        // The ReadSimpleLargeResultsScenario adds a property for fetch size.
        // This can take
        ...
        this.runScenario("Default", ReadSimpleLargeResultsScenario.class,
                props, true, 13);
    }

    /**
     * Tests off-the-shelf read random key scenario. Designed to generate 5
     * lines of csv output (1 header row + 4 individual scenario runs).
     */
    public void testReadReadScalingInvertedKeysScenario() throws Exception
    {
        ...
        this.runScenario("Default", ReadScalingInvertedKeysScenario.class,
                props, true, 5);
    }

    /**
     * Tests complex read/writes, stressing various combinations of operations
     * and number of rows selected for reads. Designed to generate 5 lines of
     * csv output (1 header row + 4 individual scenario runs).
     */
    ...
    /**
     * Tests complex writes, stressing combinations of subselected rows on
     * updates. Generates 3 lines of csv output (1 header row + 2 individual
     * scenario runs).
     */
    ...
    public void testReadScalingAggregatesScenario() throws Exception
    {
        ...
        this.runScenario("Default", ReadScalingAggregatesScenario.class, props,
                true, 5);
    }
     ...
     */
    public void runScenario(String name, Class<?> scenarioClass,
            Properties props, boolean consoleOutput, int expectedLines)
            throws Exception
    {
        ...
        assertEquals("Checking CSV output lines from benchmark run",
