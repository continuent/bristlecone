...
{
    private static final Logger logger                = Logger.getLogger(TPCBScenario.class);
    ...
    private String              nowFunction           = null;
    ...
    public void initialize(Properties properties) throws Exception
    {
        ...
        setNowFunction(properties.getProperty("nowFunction"));
    ...
    private void prepareStatements()
    {
        ...
        try
        {
            SQL = "update branch set branch_balance = branch_balance + ?, time_stamp = "
                    + getNowFunction() + " where branch_id = ?";
            branchUpdate = connection.prepareStatement(SQL);
            SQL = "update teller set teller_balance = teller_balance + ?, time_stamp = "
                    + getNowFunction() + " where teller_id = ?";
            tellerUpdate = connection.prepareStatement(SQL);
            SQL = "update account set account_balance = account_balance + ?, time_stamp = "
                    + getNowFunction() + " where account_id = ?";
            ...
            SQL = "insert into history values(?, ?, ?, ?, " + getNowFunction()
                    + ", ?)";
    ...
    private void createAndPopulate() throws SQLException
    {
        ...
        for (int i = 0; i < configuration.getNumberOfAccounts(); i++)
        {
            ...
            insertStatement.setObject(1, i, java.sql.Types.INTEGER);
            insertStatement.setObject(2,
                    i / configuration.getAccountsPerBranch(),
    ...
    public String getNowFunction()
    {
        if (nowFunction == null)
        {
            if (connection.getDialect() instanceof SqlDialectForMysql)
                return "now()";
            else if (connection.getDialect() instanceof SqlDialectForOracle)
