...
public class ConfigMetadata
{
    ...
    {
        // Scan the properties file and add metadata for any property key we
        // find.
        ...
        // Add defaults for standard metadata properties, if they are not
        // present.
    ...
    protected void addMetadataOrFail(String name, String defaultValue,
            boolean output)
    {
        boolean added = addMetadataIfAbsent(name, defaultValue, output, false,
                null);
        if (!added)
            throw new BenchmarkException(
                    "Could not add property metadata for reserved key; ensure "
                            + "this key is not used in scenario properties: key="
                            + name);
    }

    /**
     * Find a setter method that matches the return type, has a single argument,
    ...
    protected Method findSetterMethod(Class<?> c, String name)
    {
        ...
        for (Method m : methods)
        {
            if (!m.getName().equals(name))
                continue;
            /*
             * else if (! (m.getReturnType() == Void.class)) continue;
             */
    ...
    public void addMetadata(String name, String defaultValue, boolean output,
            boolean variable, Method setter)
    {
     ...
     */
    public synchronized boolean addMetadataIfAbsent(String name,
            String defaultValue, boolean output, boolean variable, Method setter)
    {
     ...
     * getting a null pointer. This can be used to fetch metadata either to read
     * it or to write it.
     */
    public ConfigPropertyMetadata getPropertyMetadataAsserted(String name)
    {
        ...
        if (cpm == null)
            throw new BenchmarkException(
                    "Attempt to access non-existent property metadata:  key="
                            + name);
        return cpm;
    }
    ...
    public void setProperties(Properties scenarioProps, Scenario scenario)
    {
        ...
        while (keys.hasMoreElements())
        {
            ...
            if (setter != null)
            {
                ...
                if (logger.isDebugEnabled())
                {
                    logger.debug("Setting scenario argument: key=" + key
                            + " value=" + value);
                }
                ...
                {
                    throw new BenchmarkException("Unable to set property: key="
                            + key + " value = " + value, e);
                }
            }
        }
