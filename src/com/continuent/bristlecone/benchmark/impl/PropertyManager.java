...
public class PropertyManager
{
     ...
     * follows the conventions described below:
     * <p>
     * <ol>
     * <li>include property. This property if present contains the relative path
     * from the current properties file to another properties file that should
     * be included.
     * <li>property value cross products. The pipe symbol within a property
     * value creates a cross product of property files.
     * </ol>
     */
    public Vector<Properties> propertiesCrossProduct(Properties props,
            File defaultDir, Vector<String> splitPropertyNames)
    {
        ...
        splitProperties(props, keys, 0, head, propertyInstances,
                splitPropertyNames);
        ...
        while (propertyIterator.hasNext())
        {
            ...
            if (include != null)
            {
                ...
                Properties includedProperties = (Properties) includedPropertiesMap
                        .get(includeFilePath);
                if (includedProperties == null)
                {
                    // We have not see this file before, so we need to read it.
                    includedProperties = loadProperties(includeFile);
                    includedPropertiesMap.put(includeFilePath,
                            includedProperties);

                    // Any property names in this file should be included as
                    // "split"
                    ...
                    {
                        Enumeration<?> includedPropertyNames = includedProperties
                                .propertyNames();
                        while (includedPropertyNames.hasMoreElements())
                        {
                            String name = (String) includedPropertyNames
                                    .nextElement();
                ...
                // Now that we have the included properties, fill in any values
                // for
     ...
     */
    public Vector<Properties> propertiesCrossProduct(File propFile,
            Vector<String> splitPropertyNames) throws BenchmarkException
    {
        // Load properties.
            ...
            Vector<String> splitPropertyNames)
    {
        if (index < keys.length)
        {
            ...
            for (int i = 0; i < values.length; i++)
            {
                ...
                node.addChild(child);
                splitProperties(props, keys, index + 1, child,
                        propertyInstances, splitPropertyNames);
            }
        }
        else
        {
